...
function Section({ title, children, accent = "default" }: {
  ...
  accent?: "default" | "rose" | "green" | "blue";
}) {
  const borderColors = {
...
export function BugDetailView({ task }: { task: Task }) {
  return (
    <article className="space-y-10">
      {/* Header */}
      <div className="space-y-4">
        <div className="inline-flex items-center rounded-full bg-rose-500/10 px-3 py-1 text-xs font-medium text-rose-600 dark:text-rose-400">
          Bug Fix
        </div>
        ...
        {task.summary && (
          <p className="text-base text-muted-foreground leading-relaxed border-l-2 border-rose-500/30 pl-4">
            {task.summary}
          </p>
        )}
      </div>
      ...
      {/* Problem */}
      <Section title="Problem" accent="rose">
      ...
      {task.changedItems.length > 0 && (
        <Section title="Changed Files & Rules" accent="blue">

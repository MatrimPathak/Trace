...
function Section({ title, children, accent = "default" }: {
  ...
  accent?: "default" | "amber" | "green" | "blue" | "orange";
}) {
  const borderColors = {
...
export function SpikeDetailView({ task }: { task: Task }) {
  return (
    <article className="space-y-10">
      {/* Header */}
      <div className="space-y-4">
        <div className="inline-flex items-center rounded-full bg-amber-500/10 px-3 py-1 text-xs font-medium text-amber-600 dark:text-amber-400">
          Spike · Research
        </div>
        ...
        {task.summary && (
          <p className="text-base text-muted-foreground leading-relaxed border-l-2 border-amber-500/30 pl-4">
            {task.summary}
          </p>
        )}
      </div>
      ...
      {/* Research Goal */}
      <Section title="Research Goal" accent="amber">
        <Prose text={task.problem} />
      </Section>

      {/* Evaluated Approaches & Findings */}
      <Section title="Findings & Evaluated Approaches">
        <Prose text={task.implementation} />
      </Section>

      {/* Architecture Diagrams / Screenshots */}
      {task.screenshots.length > 0 && (
        <Section title="Diagrams & References">
      ...
      {task.changedItems.length > 0 && (
        <Section title="Systems Considered" accent="blue">
          <ChangedItemPills items={task.changedItems} />
        </Section>
      )}

      {/* Recommendations / Impact */}
      <Section title="Recommendation" accent="green">
        <Prose text={task.impact} />
      </Section>

      {/* Final Decision / Learnings */}
      <Section title="Final Decision & Learnings" accent="orange">
